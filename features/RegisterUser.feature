Feature: Registro de Usuario
  ...
  Scenario: El usuario desea crear una nueva cuenta en la plataforma
    ...
    Then el sistema debe permitirle finalizar el registro y mostrar un mensaje de confirmación de cuenta creada

  Scenario: El usuario utiliza un correo previamente registrado
    Given que el usuario se encuentra en la sección de registro
    When intenta registrarse con una dirección de correo que ya está vinculada a otra cuenta
  ...
  Scenario: El usuario omite campos obligatorios en el formulario
    Given que el usuario se encuentra en la sección de registro
    When intenta registrarse sin proporcionar toda la información requerida
